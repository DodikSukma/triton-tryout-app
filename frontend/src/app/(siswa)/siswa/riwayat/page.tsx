  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto">
      <header className="mb-8">
        <h1 className="text-3xl font-black text-slate-900">Riwayat & Nilai</h1>
        <p className="text-slate-500 mt-1">Pantau perkembangan belajar Anda dari semua tryout.</p>
      </header>
      ...
      {rows.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center">
          ...
          <p className="text-slate-500 text-lg font-medium">Belum ada riwayat tryout</p>
          <p className="text-slate-400 text-sm mt-1">Mulai kerjakan tryout pertamamu untuk melihat riwayat di sini.</p>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.sesi_id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-5 py-4 font-semibold text-slate-900">
                        {r.nama_tryout ?? <span className="italic text-slate-400">Tryout tidak tersedia</span>}
                      </td>
                      <td className="px-5 py-4 text-slate-500 hidden md:table-cell">{r.mata_pelajaran ?? '—'}</td>
                      ...
                      <td className="px-5 py-4 text-slate-500 text-xs hidden md:table-cell inline-flex items-center gap-1">
                        <Calendar size={11} />
...
function StatCard({ label, value, Icon, color, suffix }: {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
      ...
      <p className="text-2xl font-black text-slate-900 mt-3 tabular-nums">{value}{suffix && <span className="text-sm text-slate-400 font-medium">{suffix}</span>}</p>
